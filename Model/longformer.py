...
class LongformerBartEncoderWithDocType(BartPretrainedModel):
    def __init__(self,
        ...
        if doc_type_tokens is not None:
            ...
            self.doc_type_tokens = nn.Embedding(config.doc_type_size,embed_dim, self.padding_idx)
...
class BartDecoderWithDocType(BartPretrainedModel):
    ...
    def forward(
        ...
        r"""
        BartDecoder + add doc_type_embedding 
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        doc_type = self.doc_type_tokens(doc_type_ids)
        hidden_states = inputs_embeds + positions + doc_type
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # layer_outputs : output, attn_weight, past_key_value
            hidden_states = layer_outputs[0] # output
