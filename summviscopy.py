...
if __name__ == "__main__":
    ...
    nlp, is_lg = get_nlp() # is_lg -> False

    Spacy.encode = _spacy_encode # function
    Spacy.decode = _spacy_decode # function
    rg_spacy = Spacy(nlp=nlp)

    path = Path(args.path) #PosixPath('data') <- args.path=='data'
    all_files = set(map(os.path.basename, path.glob('*'))) # {'cnn_dailymail_10.validation'}
    files = sorted([
        fname for fname in all_files if not (fname.endswith(".py") or fname.startswith("."))
    ]) # ['cnn_dailymail_10.validation']
    if args.file:
        try:
        ...
        file_index = 0
    col1, col2 = st.beta_columns((3, 1)) # beta_columns -> 화면 레이아웃 나누는 것
    filename = col1.selectbox(label="File:", options=files, index=file_index)
    dataset = load_dataset(str(path / filename)) # RGDataset[num_rows: 10](RGDataset(jsonl=preloading/cnn_dailymail_1000.validation.predictions.jsonl))
